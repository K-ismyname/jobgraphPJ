...
- **GitHub·HF Spaces 둘 다 3주 만에 갱신** — 두 원격 모두 2026-07-03 시점에 멈춰 있었다.
  `git subtree split --prefix=pj1` 결과가 기존 원격 tip을 조상으로 포함해 **fast-forward로 push**
  (강제 덮어쓰기 불필요). 라이브에 처음 반영된 것 — SSRF 가드, 관리자 fail-closed, IP별 일일 상한,
  미분류 공고 오염 차단, 근거 정직성(`graph_only`), 골든셋, 노이즈 필터 배선, `test_wiring.py`
- HF Space 재빌드 후 `stage: RUNNING`, 홈·`/observe` 모두 HTTP 200 확인

### 배포 후 발견 — Neo4j Aura 인스턴스가 일시정지 상태

라이브 검증 중 `/jobs/trending-skills`가 `"데이터베이스에 연결할 수 없습니다"` 를 반환했다.
로컬에서 같은 인스턴스에 붙어 원인을 좁혔다.

- 일반 DNS는 정상(`github.com`, `databases.neo4j.io` 모두 해석됨)인데 **Aura 인스턴스 호스트만
  해석 실패** → 잠든 게 아니라 **일시정지(paused)** 상태
- 마지막 DB 활동은 2026-07-13(노이즈 38개 제거). **23일 미사용** → Aura 무료 티어 자동 일시정지
- **배포와 무관한 기존 상태**다. 코드 배포는 정상이고 앱도 떠 있다. 다만 **Aura 콘솔에서 인스턴스를
  재개하기 전까지 라이브 데모의 분석 기능은 동작하지 않는다.**
- 주의: Aura 무료는 일시정지 후 방치하면 결국 삭제된다. **604건 데이터가 사라지면 현재 코드로는
  재현할 수 없다**(수집 스크립트 `collect_and_merge.py`·`remoteok_client.py`가 삭제된 상태 — M-5).
  재개 우선순위가 생각보다 높다.
