...
# LinkedIn 리다이렉트/스팸 패턴 — description이 실제 공고 내용 없이 잘린 경우
_SPAM_PATTERNS = re.compile(
    r"similar jobs on LinkedIn"          # "See this and similar jobs on LinkedIn"
    r"|kahalintulad na mga trabaho sa"   # 타갈로그어 LinkedIn 리다이렉트
    r"|Job openings at \w",              # "Job openings at JumpCloud" 류 회사 페이지
    re.IGNORECASE,
)

# 개발자 직무 타이틀 allowlist — RemoteOK는 비개발자 공고가 dev 태그로 혼입됨
_DEV_TITLE_KEYWORDS = frozenset([
    "engineer", "developer", "programmer", "devops", "sre",
    "architect", "platform", "backend", "frontend", "fullstack",
    "full-stack", "data scientist", "data analyst", "data engineer",
    "machine learning", "software", "infrastructure", "cloud",
    "security", "systems", "database", "dba", "tech lead",
    "staff eng", "principal eng", "ml ", " ml",
])


def _fix_encoding(text: str) -> str:
    """UTF-8 문자가 latin-1로 잘못 해석된 문자열을 복원.

    RemoteOK API는 가끔 description 내 UTF-8 특수문자(', …, —)를
    latin-1 바이트로 잘못 디코딩한 채 응답한다.
    encode('latin-1').decode('utf-8') 역변환으로 원래 문자를 복원한다.
    """
    try:
...
def _is_dev_title_remoteok(title: str) -> bool:
    """개발자 관련 타이틀인지 allowlist 방식으로 확인."""
    t = title.lower()
    return any(kw in t for kw in _DEV_TITLE_KEYWORDS)


def preprocess_remoteok_job(raw: dict) -> dict:
    """RemoteOK 원본 공고 1개를 파이프라인 중간 포맷으로 변환.

    The Muse의 preprocess_job()과 동일한 출력 포맷을 반환해
    이후 스킬 추출·Neo4j 적재 파이프라인을 공용으로 쓸 수 있다.
    """
    raw_description = raw.get("description", "")
    description = _fix_encoding(raw_description) if raw_description else ""

    text_clean = strip_html(description)
    required_section, preferred_section = extract_sections(description)
    ...
    # RemoteOK는 0을 "정보 없음"으로 사용
    if salary_min == 0:
        salary_min = None
    if salary_max == 0:
        salary_max = None

    raw_location = raw.get("location", "").strip()

    return {
        "id": f"remoteok-{raw.get('id', raw.get('slug', ''))}",
        "title": raw.get("position", ""),
        "company": raw.get("company", ""),
        "location": raw_location if raw_location else "Remote",
        "level": "",
        "url": raw.get("url", raw.get("apply_url", "")),
        "created": raw.get("date", "2025-01-01T00:00:00Z"),
        ...
        "preferred_section": preferred_section,
        "bullet_section": bullet_section,
        "category": raw.get("_collected_tag", ""),
        "source": "remoteok",
    }


def preprocess_remoteok_file(
    input_path: str | Path,
    output_path: str | Path,
) -> list[dict]:
    """RemoteOK raw JSON 파일을 읽어 전처리 후 저장.

    필터 3단계:
    1. 개발자 타이틀 아닌 공고 제거 (allowlist)
    2. description이 너무 짧은 공고 제거 (스팸/링크드인 리다이렉트)
    3. (title+company) 중복 제거
    """
    input_path = Path(input_path)
    output_path = Path(output_path)
    ...
    # 1. 개발자 타이틀 필터
    raw_jobs = [j for j in raw_jobs if _is_dev_title_remoteok(j.get("position", ""))]
    filtered_non_dev = before - len(raw_jobs)

    # 2. LinkedIn 리다이렉트/스팸 공고 제거
    raw_jobs = [
        j for j in raw_jobs
        if not _SPAM_PATTERNS.search(j.get("description", ""))
    ]
    filtered_short = (before - filtered_non_dev) - len(raw_jobs)

    processed_all = [preprocess_remoteok_job(j) for j in raw_jobs]
    ...
    with_req = sum(1 for j in processed if j["required_section"])
    with_pref = sum(1 for j in processed if j["preferred_section"])
    print(f"{before}개 → 비개발자 -{filtered_non_dev} / 짧은설명 -{filtered_short} / 중복 -{filtered_dup} → {total}개")
    if total > 0:
        print(f"  required 섹션 파싱: {with_req}/{total} ({with_req/total*100:.0f}%)")
        print(f"  preferred 섹션 파싱: {with_pref}/{total} ({with_pref/total*100:.0f}%)")
