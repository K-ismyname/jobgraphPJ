...
**아직 측정하지 못한 것 (가장 중요한 한계)**

Faithfulness는 "말한 것이 근거에 있는가"만 잰다. **갭 목록 자체가 맞는가**(진짜 부족한 스킬인가,
놓친 스킬은 없는가)는 어떤 지표도 재지 않는다. critic도 consensus와의 *일관성*만 볼 뿐 *현실과의
일치*는 못 본다. 이걸 재려면 사람이 라벨링한 골든셋(이력서 × 직군 → 정답 갭 목록)으로
precision/recall을 측정해야 하며, **이것이 다음 최우선 과제다.**
